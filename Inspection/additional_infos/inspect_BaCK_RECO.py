...
for jobDir in sorted(os.listdir(options.path)):
    ...
    try:
        ...
        # reco and sim trees are positionally 1:1 per job (same entry count,
        # same order); GetMCEntryNumber()/GetEventID() are global IDs, not
        # local row indices, so index both trees by the loop position instead.
