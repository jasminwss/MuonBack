# reco/sim matching: loop is driven by the reco tree, sim is pulled to the same
# row with tree_sim.GetEntry(eventNr) (GetEntry only ever on the tree we are NOT
# iterating). Only truth quantities (tree_sim.*) are used below; the reco event
# is kept solely for the entry-level sanity assert.
...
def print_result(tag):
	...
	with open(directory +tag+'_readme.txt', 'w') as readme: 
		...
		# cell can be dominated by a single very-high-weight muon, so the 2nd cell is
...
for jobDir in sorted(os.listdir(options.path)):
    ...
    if not reco_files or not sim_files or not geo_files:
        print(f"Skipping {jobDir}: missing reco, sim or geo file")
        continue

    f, f_sim, fgeo = None, None, None
    try:
        # Geo einmal laden
        if sGeo is None:
        ...
        # reco and sim are positionally 1:1 per job (same entry count, same
        # order), so both trees are indexed by the loop position. Drive the
        # loop off the reco tree and pull sim to the same row with GetEntry
        # on the *other* tree - never GetEntry the tree you are iterating,
        # that overwrites the iterator's own buffers and you silently read a
        # different event than you think.
        for eventNr, event in enumerate(tree):
            tree_sim.GetEntry(eventNr)

            # event.*    -> reco  (ShipEventHeader, reconstructed hits, ...)
            # tree_sim.*  -> truth (MCTrack, vetoPoint, MCEventHeader, ...)
            # do not mix the two names: event.vetoPoint does not exist anymore.
            assert event.ShipEventHeader.GetMCEntryNumber() == tree_sim.MCEventHeader.GetEventID(), \
                f'sim/reco mismatch at entry {eventNr}'
